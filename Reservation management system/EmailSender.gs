 ...
 * キャンセル確認メール送信
 */
function sendCancellationConfirmationEmail(reservation) {
  const subject = '【キャンセル完了】訪問予約をキャンセルしました';
  
  let body = `${reservation.name} 様\n\n`;
  body += `予約ID: ${reservation.reservationId}\n`;
  body += `予約のキャンセルが完了しました。\n\n`;
  body += `キャンセル日時: ${formatDateTime(new Date())}\n\n`;
  body += `またのご利用をお待ちしております。`;
  
  GmailApp.sendEmail(reservation.email, subject, body);
  Logger.log('キャンセル確認メール送信: ' + reservation.email);
}
